...
const carregarPerfil = async () => {
  try {
    ...
    let { data, error } = await supabase.from('aluno').select('*').eq('auth_id', s.user.id).maybeSingle()
    if (!data && !error) {
      const res = await supabase.from('aluno').select('*').eq('email', s.user.email).maybeSingle()
...
const cancelarEdicao = () => { form.value = { ...aluno.value }; editando.value = false; errorMsg.value = '' }
...
const salvar = async () => {
  ...
  try {
    const { error } = await supabase.from('aluno').update({
      ...
      cpf: form.value.cpf, telefone: form.value.telefone,
  ...
  } catch (e) { errorMsg.value = 'Erro ao salvar: ' + e.message }
...
const alterarSenha = async () => {
  ...
  try {
    ...
    setTimeout(() => successMsg.value = '', 3000)
  } catch (e) { errorMsg.value = 'Erro: ' + e.message }
...
const handleFotoChange = async (e) => {
  ...
  try {
    ...
    await supabase.from('aluno').update({ foto: urlData.publicUrl }).eq('idaluno', aluno.value.idaluno)
    aluno.value.foto = urlData.publicUrl
